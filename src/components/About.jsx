...
const About = () => {
  ...
  useEffect(() => {
    if (isMobile) {
      ...
      setSlidesToShow(4); // Set slidesToShow to 3 for larger screens
    } else {
          ...
          <div className='mt-20 gap-5'>
            <Slider {...settings}>
              {technologies.map((technology, index) => (
                <img key={index} src={technology.icon} alt={technology.name} className='max-w-[80px] h-[80px] gap-5 ml-15' />
              ))}
            </Slider>
          </div>
